...
def disaggregate_temporal(
    ...
    if not force_preprocessing:
        consumption_disaggregate_temporal = (
            load_consumption_disaggregate_temporal_cache(
                sector=sector, energy_carrier=energy_carrier, year=year
            )
        )

        if consumption_disaggregate_temporal is not None:
            logger.info(
                f"Loading from cache: disaggregate_temporal(sector={sector}, energy_carrier={energy_carrier}, year={year})"
            )
            return consumption_disaggregate_temporal

    # 1. Get the consumption data with efficiency factor
    consumption_data = disagg_applications_efficiency_factor(
        sector=sector,
    ...
    if sector == "industry":
        ...
        consumption_disaggregate_temporal = disaggregate_temporal_industry(
        ...
        if energy_carrier == "gas":
            # sum over the applications but with efficiency factor
            consumption_data = consumption_data.T.groupby(level=0).sum().T
            consumption_disaggregate_temporal = disagg_temporal_heat_CTS(
                consumption_data=consumption_data, year=year
            )

        elif energy_carrier == "power":
            # sum over the applications but with efficiency factor
            consumption_data = consumption_data.T.groupby(level=0).sum().T
            consumption_disaggregate_temporal = disaggregate_temporal_power_CTS(
                consumption_data=consumption_data, year=year
            )
            ...
            # resolve with SLPs
            consumption_data = consumption_data.T.groupby(level=0).sum().T
            consumption_disaggregate_temporal = disagg_temporal_petrol_CTS(
                consumption_data=consumption_data, year=year
            )

    # sanity check
    if not np.isclose(
        consumption_disaggregate_temporal.sum().sum(),
        consumption_data.sum().sum(),
        atol=1e-6,
    ):
        raise ValueError(
            f"The sum of the disaggregated temporal consumption is not equal to the sum of the initial consumption data for {sector} and {energy_carrier} in year {year}"
        )
    if consumption_disaggregate_temporal.isna().any().any():
        raise ValueError(
            f"The disaggregated temporal consumption contains NaN values for {sector} and {energy_carrier} in year {year}"
        )

    # 2. save to cache
    if save_to_cache:
        logger.info("Saving to cache...")
        processed_dir = load_config("base_config.yaml")[
        ...
        processed_file = os.path.join(
            ...
            ].format(energy_carrier=energy_carrier, year=year, sector=sector),
        )
        os.makedirs(processed_dir, exist_ok=True)
        consumption_disaggregate_temporal.to_csv(
            processed_file, float_format=f"%.{float_precision}f"
        )
        logger.info(f"Saved to {processed_file}")

    logger.info(
        f"Disaggregation of temporal consumption for {sector} and {energy_carrier} in year {year} completed."
    )
