...
def disagg_applications_efficiency_factor(
    sector: str,
    energy_carrier: str,
    year: int,
    ...
    if not force_preprocessing:
        consumption_data_with_efficiency_factor = (
            load_consumption_data_with_efficiency_factor_cache(
        ...
        if consumption_data_with_efficiency_factor is not None:
            logger.info(
                f"Loading from cache: disagg_applications_efficiency_factor(sector={sector}, energy_carrier={energy_carrier}, year={year})"
            )
    ...
    consumption_data_sectors_regional = (
        get_consumption_data_per_indsutry_sector_energy_carrier(
            year=year,
            cts_or_industry=sector,
    ...
    consumption_data_dissaggregated = dissaggregate_for_applications(
        consumption_data=consumption_data_sectors_regional,
        year=year,
        sector=sector,
        energy_carrier=energy_carrier,
    )
    ...
    # 5. sanity check
    if not np.isclose(
        consumption_data_dissaggregated.sum().sum(),
        consumption_data_sectors_regional.sum().sum(),
    ):
        raise ValueError(
    ...
    consumption_data_with_efficiency_factor = apply_efficiency_factor(
        consumption_data=consumption_data_dissaggregated,
        sector=sector,
        energy_carrier=energy_carrier_eff,
        year=year,
    )
    ...
    logger.info(
        f"Cached: disagg_applications_efficiency_factor(sector={sector}, energy_carrier={energy_carrier}, year={year})"
    )
